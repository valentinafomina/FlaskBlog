...
from FlaskBlog.auth.utils import save_picture, send_reset_email
...
def account():
    ...
    return render_template('userprofile.html', title='Аккаунт',
...
def user_posts(username):
    ...
    posts = Post.query.filter_by(author=user)\
        .order_by(Post.date_posted.desc())\
        .paginate(page=page, per_page=5)
    user_pic = url_for('static', filename='profile_pics/' +
                                            current_user.image_file)
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('token_reset.html', title='Reset password',
                           form=form)
